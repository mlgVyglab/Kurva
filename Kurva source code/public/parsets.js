...
function drawParsets(){

  detailsArea.remove()

  
  
  columnas = [data.columns[4],data.columns[5],data.columns[6],data.columns[7],data.columns[8],data.columns[9],data.columns[10]]

  var chart = d3.parsets().dimensions(columnas);

  detailsArea = d3.select("#details_div").append("svg")
                                    .attr("width",d3.select("#details_div").node().clientWidth)
                                    .attr("height",d3.select("#details_div").node().clientHeight);


  detailsArea.datum(data).call(chart);
